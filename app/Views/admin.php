    ...
    <div class="p-8 shadow-md max-w-5xl w-full mx-auto backdrop-blur-sm bg-transparent rounded-lg">
        ...
        <div class="w-full mx-auto border-2 rounded-lg border-emerald-400 mb-8">
            <div class="px-6 py-4 bg-transparent rounded-t-lg">
                <h2 class="text-2xl font-semibold text-center">User List</h2>
            </div>
        ...
        <div class="w-full mx-auto border-2 rounded-lg border-emerald-400">
            <div class="px-6 py-4 bg-transparent rounded-t-lg">
                <h2 class="text-2xl font-semibold text-center">Device Management</h2>
            </div>
            <div class="p-6 bg-transparent rounded-b-lg">
                <table class="w-full table-auto">
                    <thead class="bg-transparent">
                        <tr>
                            <th class="px-4 py-2 text-left font-medium">Device ID</th>
                            <th class="px-4 py-2 text-left font-medium">Device Name</th>
                    ...
                    <tbody>
                        <?php foreach ($dispositivos as $device): ?>
                            <tr class="hover:bg-transparent transition duration-150 ease-in-out">
                                ...
                                <td class="border-t border-gray-200 px-4 py-2">
                                    <a href="<?= site_url('edit_device/' . $device['id_dispositivo']); ?>" class="bg-blue-500 hover:bg-blue-600 text-white font-bold py-1 px-3 rounded text-sm">Edit</a>
                                    <a href="<?= site_url('delete_device/' . $device['id_dispositivo']); ?>" class="bg-red-500 hover:bg-red-600 text-white font-bold py-1 px-3 rounded text-sm" onclick="return confirm('¿Estás seguro de que deseas eliminar este dispositivo?');">Delete</a>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
